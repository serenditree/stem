...
_VOLUME_DST=/bitnami/elasticsearch/data
_EXPOSE=9200/tcp
...
########################################################################################################################
    ...
    podman run \
        ...
        --name ${_CONTAINER}-1 \
        --env-file ./plot.env \
        --env S3_ACCESS_KEY="$(pass serenditree/iam/backup@exoscale.com.access)" \
        --env S3_SECRET_KEY="$(pass serenditree/iam/backup@exoscale.com.secret)" \
        --volume ./rc/elasticsearch.yml:/opt/bitnami/elasticsearch/config/elasticsearch.yml:Z \
    ...
    sc_container_rm ${_CONTAINER}-2
    podman run \
        --log-level $_ST_LOG_LEVEL \
        --pod $_ST_POD \
        --name ${_CONTAINER}-2 \
        --env-file ./plot.env \
        --label serenditree.io/service=kibana \
        --health-cmd "curl http://localhost:5601/api/task_manager/_health" \
        --health-interval 3s \
        --health-retries 3 \
        --detach \
        "docker.io/bitnami/kibana:${_ST_VERSION_ELASTIC}"
fi
