...
# ARG_OPTIONAL_BOOLEAN([issuer_override], [i], [Set cert-issuer to prod when stage is not prod and vice versa.])
...
export _ARG_ISSUER_OVERRIDE=${_arg_issuer_override//off/}
...
function sc_help() {
    ...
    printf '\t%-20s%s\n' "up|uc|u [svc]:" "Starts a local development stack or a single container. $_options"
    _options='[--compose] [--integration]'
    printf '\t%-20s%s\n\n' "down|d [svc]:" "Stops local stack or single containers. $_options"
