...
function sc_test_generate_openapi() {
    local -r _volume_src=${_ST_HOME_STEM}/charts/terra/testing/generated
...
function sc_test_generate_har() {
    ...
    local -r _js="${_ST_HOME_STEM}/charts/terra/testing/generated/har-$(date +%Y%m%d-%H%M%S).js"
...
function sc_test_run_local() {
    ...
    if k6 &>/dev/null; then
        k6 run $_env "${_ST_HOME_STEM}/charts/terra/testing/resources/main.js"
    else
        podman run \
            --rm \
            --network host \
            --volume "${_ST_HOME_STEM}/charts/terra/testing/resources/:/home/k6/:Z" \
...
function sc_test_run_cluster() {
    ...
    if [[ -z "$_ARG_DELETE" ]]; then
        ...
        kubectl create \
            --filename "${_ST_HOME_STEM}/rc/jobs/test-serenditree.yml" \
