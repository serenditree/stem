...
function sc_status_cluster() {
    ...
    exo storage ls --use-account serenditree --versions sos://serenditree-backup-user/data/ |
        head -n $_head |
        column -t &&
        echo
    exo storage ls --use-account serenditree --versions sos://serenditree-backup-seed/data/ |
        grep 'data/index.latest' |
        head -n $_head |
        column -t
