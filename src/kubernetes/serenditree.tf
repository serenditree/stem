...
resource "helm_release" "serenditree" {
  ...
  ######################################################################################################################
  set_sensitive {
    name  = "rootSeed.parameters.snapshot.accessKey"
    value = exoscale_iam_api_key.serenditree_backup.key
  }
  set_sensitive {
    name  = "rootSeed.parameters.snapshot.accessSecret"
    value = exoscale_iam_api_key.serenditree_backup.secret
  }
}
