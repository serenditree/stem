...
function sc_pod_up() {
    ...
    if [[ -n "${_ARG_WATCH}${_ARG_INTEGRATION}" ]]; then
        ...
        local _iteration=0
        until sc_pod_health $(( ++_iteration )); do :; done
        if [[ $? -ne 0 ]]; then
...
function sc_pod_health() {
    ...
    # latest (actively run health-checks)
        ...
        local -r _count=$(podman ps --noheading --filter label=serenditree.io/service | wc -l)
        for _container in $(podman ps --filter label=serenditree.io/service --format '{{.Names}}'); do
            ...
            echo -e "\e[2K${_iteration} ${_container}: $_status"
        done
        [[ $_exit -eq 1 ]] && echo -en "\e[${_count}A"
    fi
    return $_exit
}
