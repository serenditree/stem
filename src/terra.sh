...
function sc_terra_up_wait() {
    ...
    local _state=
    local _reset_done=
    local _terminate_done=

    until [[ $_healthy -eq $_total ]]; do
        (( ++_iteration ))
        sc_cluster_expose argocd &>>$_tmp

        _state="$(argocd app list 2>>$_tmp | sed 1d)"
        if grep -Eq " (OutOfSync|Synced|Unknown|Missing) " <<<"$_state"; then
            _total=$(wc -l <<<"$_state")
            _healthy=$(grep -v "Error" <<<"$_state" | grep -c " Healthy ")

            echo -en "$_state\nHealthy: $_healthy/$_total Iteration: $_iteration"

            # Refresh apps with defined states
            sed -En "s/^(\S+).* $_refresh .*/\1/p" <<<"$_state" |
                xargs -r -I{} bash -c "argocd app get --hard-refresh {}; argocd app sync --replace --async {}" &>>$_tmp

            # Reset certificate request
            [[ -z "$_reset_done" ]] &&
                grep -Eq "${_ns}/branch.* Healthy .*" <<<"$_state" &&
                grep -Eq "${_ns}/terra-gateway.* Degraded .*" <<<"$_state" &&
                _ARG_RESET=on sc_cluster_certificate &>>$_tmp &&
                echo -n " Reset certificate request." &&
                _reset_done=on

            # Trigger sync operation of app of apps
            [[ -z "$_terminate_done" ]] &&
                [[ $(( _total - _healthy )) -eq 1 ]] &&
                argocd app terminate-op ${_ns}/serenditree &>>$_tmp &&
                argocd app sync ${_ns}/serenditree &>>$_tmp &&
                echo -n " Triggered app of apps sync." &&
                _terminate_done=on

            # Wait and clear state
            [[ $_healthy -ne $_total ]] &&
                sleep 4s &&
                echo -en "\r\e[$(( _total ))A"
        else
            sleep 2s
        fi
    done
...
function sc_terra_down() {
    ...
    if sc_prompt "Destroy terraform resources?"; then
        for _cmd in "destroy -compact-warnings -target=exoscale_nlb.serenditree" "destroy"; do
