...
function sc_cluster_restore() {
    ...
    # Get latest snapshot
    kubectl port-forward --namespace serenditree svc/root-seed 9200:9200 &
    local -r _pid=$!
    sleep 1s
    local -r _latest="$(
        curl -s 'http://localhost:9200/_snapshot/seed-backup/_all?size=1&order=desc' |
            jq -r '.snapshots[0].snapshot'
    )"
...
function sc_cluster_expose() {
    ...
    done <"${_ST_HOME_STEM}/rc/plain/cluster-expose"; } |
        column -ts';' |
        tr ' ' '.'
    echo -e "\nCheck logs in ${_logs}!"
}
...
function sc_cluster_down() {
    ...
    sc_heading 2 "Removing traces..."
    exo storage rm --recursive --force sos://serenditree-traces
}
